...
- Coloring page picker (added 2026-08-08): each drawing card on `/drawings` has a "🖍️ Coloring Page" button. Runs the drawing photo through a client-side canvas edge-detect filter (`ColoringPageModal.tsx`) to make a printable black-and-white line-art version, with a Print button that opens a print-ready popup. No new assets needed, works off the existing drawing photos.
